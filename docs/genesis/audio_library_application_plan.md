...
- **[Done]** CI for both platforms — fixed a real, previously-latent break during the V1 readiness pass: `cargo test --workspace` failed to even compile on a clean checkout of *either* platform, because `crates/similarity-worker`'s sidecar binary (gitignored, machine-built) was never built before tests ran in CI. `ci.yml` now builds it first (`scripts/build-similarity-worker-sidecar.sh`); see `docs/src/content/docs/development/windows-setup.md` for the equivalent local, one-time step.
