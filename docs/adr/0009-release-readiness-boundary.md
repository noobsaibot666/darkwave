...
Darkwave tracks release readiness as an explicit gate model in `release-readiness`. A `ReleaseReadinessConfig` aggregates code-owned gates with optional distribution metadata before producing a release candidate. Code-owned gates include accessibility preferences, recovery prompts, performance profiling status, onboarding/documentation coverage, validation for configured update channel metadata, validation for signing identity metadata, and validation that packaged codec coverage and license review metadata exist. Distribution-owned gates still include actual update channel provisioning, codec bundle shipping, and signing/notarization until certificates, decoder artifacts, and platform release credentials are configured.
