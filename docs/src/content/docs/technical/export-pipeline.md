...
The planner remains separate from execution. Original-file copies can be executed directly, held in a queue until offline source paths become available, or exposed as OS-facing drag payloads after the destination copy exists. WAV exports can be rendered once a decoder supplies PCM; the renderer resamples to the target rate, writes 24-bit WAV data, applies selected ranges, and exposes completed render outputs as external drag payloads. `audio-metadata` exposes a packaged-decoder provider interface for arbitrary supported formats; release builds still need real decoder artifacts before those formats can render.

The desktop shell wires the original-copy path (`export_selected_asset`) and the CSV license report (`export_project_license_report`, mapping `storage::project_source_report` rows onto `render_license_report_csv` and writing the result to a path the user picks). The queued export state, the WAV renderer, and both external-drag-payload builders are not wired to the shell: the queue has no offline-source scenario to react to yet, and rendering a WAV requires a format-choice UI the shell doesn't have.
