...
- Decoded-PCM WAV rendering for 24-bit exports, including selected range slicing.
...
The planner remains separate from execution. Original-file copies can be executed directly, held in a queue until offline source paths become available, or exposed as OS-facing drag payloads after the destination copy exists. WAV exports can be rendered once a decoder supplies PCM matching the target sample rate; the renderer writes 24-bit WAV data, applies selected ranges, and exposes completed render outputs as external drag payloads. Decoder and resampler attachment remain backend follow-up work.
