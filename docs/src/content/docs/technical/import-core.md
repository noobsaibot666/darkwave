...
Import no longer enqueues a `Hashing` job — the content hash is already a full-file SHA-256 computed synchronously above, so there is no separate hashing work for a queue to do. The `MetadataExtraction` job it does enqueue is processed automatically right after import: the shell's `process_pending_jobs` command extracts embedded WAV `LIST/INFO` metadata (title, genre, comment) for each pending job and marks it complete. `WaveformGeneration` jobs are completed reactively, when the frontend finishes computing real waveform peaks for an asset during playback. There is still no generic, always-running background worker with retry/priority/throttling — see ADR 0021.
