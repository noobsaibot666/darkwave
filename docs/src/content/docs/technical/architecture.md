...
- `audio-metadata` extracts immediate filesystem metadata, exposes codec support diagnostics, and decodes native WAV PCM previews.
...
- `backup` owns backup package metadata and restore input validation.
