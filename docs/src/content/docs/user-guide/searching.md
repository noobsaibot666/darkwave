...
- A Format filter next to the range filters, covering every format the decoder accepts (WAV, MP3, FLAC, AAC, M4A, OGG, AIFF), read from each file's own extension rather than a stored field.
