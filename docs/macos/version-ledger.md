...
|------|---------|-------|--------|-------|
| 2026-09-13 | 0.3.1 | 13 | Built | Fixes a critical regression: analyzed tracks silently vanished from the canvas after swapping the active library mid-batch (a catalog-swap race in the analysis/waveform/instrument job pipeline, made easy to hit by 0.3.1/12's new File > Open Library / Last Open menu items). Not yet uploaded — update this row once Transporter tells you the outcome. Direct-sale build for the same commit is also done (notarized + stapled) but held out of Store Manager's ingest per this repo's own gating rule until Transporter confirms this row. |
