...
|------|---------|-------|--------|-------|
| 2026-09-13 | 0.3.1 | 13 | Uploaded | Fixes a critical regression: analyzed tracks silently vanished from the canvas after swapping the active library mid-batch (a catalog-swap race in the analysis/waveform/instrument job pipeline, made easy to hit by 0.3.1/12's new File > Open Library / Last Open menu items). Transporter log confirmed `CREATE BUILD` 201 (created) for cfBundleShortVersionString 0.3.1 / cfBundleVersion 13, and the asset description upload reached `assetDeliveryState.state: COMPLETE` — delivery to App Store Connect succeeded. Still waiting on Apple's review decision. Direct-sale build for the same commit is also done (notarized + stapled). |
| 2026-09-13 | 0.3.1 | 12 | Uploaded | Redesigned New Setup's sound-files step, added File > Open Library / Last Open, fixed the .darkwave document icon, and wired the version ledger into every build script (see this row's own history above). Transporter log confirmed `CREATE BUILD` 201 (created) for cfBundleShortVersionString 0.3.1 / cfBundleVersion 12, and the asset description upload reached `assetDeliveryState.state: COMPLETE` — delivery to App Store Connect succeeded. Still waiting on Apple's review decision. |
